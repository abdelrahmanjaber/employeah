 ...
 * * LOGIC FIX:
 * The percentage is calculated RELATIVE to the filters provided.
 * If jobField="Backend Developer" and location="London", the denominator
 * is ONLY Backend Developer jobs in London.
 */
export async function getSkillTrendData(skillName, jobField = "", location = "") {
  await delay(300);
  if (!skillName) return [];

  const monthlyStats = {};

  // 1. First, define the "Universe" of jobs we care about based on filters
  const relevantJobs = JOBS_DEMO.filter(job => {
    // If a job field is specified, it MUST match
    if (jobField && job.title.toLowerCase() !== jobField.toLowerCase()) return false;
    
    // If a location is specified, it MUST match
    if (location && !job.location.toLowerCase().includes(location.toLowerCase())) return false;
    
    return true;
  });

  // 2. Now process only this subset to calculate trends
  relevantJobs.forEach(job => {
    ...
    if (!monthlyStats[key]) {
      monthlyStats[key] = { totalJobsInUniverse: 0, skillMatches: 0 };
    }

    // Increment denominator (This is a job in our targeted field/location)
    monthlyStats[key].totalJobsInUniverse += 1;
    
    // Check if it has the specific skill (Numerator)
  ...
  return Object.entries(monthlyStats)
    ...
    .map(([key, stats]) => {
      ...
      // Calculate Percentage: (Jobs with Skill / Total Jobs in Field&Loc) * 100
      const percentage = stats.totalJobsInUniverse > 0 
        ? (stats.skillMatches / stats.totalJobsInUniverse) * 100 
        : 0;
...
/**
 * NEW: Get Skill Distribution (Share of Voice)
 * Calculates DISTRIBUTION: (Skill Count / Total Skill Mentions)
 * Result sums to exactly 100%. Use this ONLY for Pie Charts.
 */
export async function getSkillDistributionForJob({ job, location } = {}) {
  ...
  const skillsCount = {};
  let totalSkillMentions = 0; // Count every single tag found across all jobs
  ...
  Object.entries(skillsCount)
    ...
    .forEach(([skill, count]) => {
      skillsStats[skill] = {
        count,
        percentage: Number(((count / totalSkillMentions) * 100).toFixed(1))
      };
    });
  ...
  return {
    ...
    job,
    location,
    total_jobs: matchingJobs.length,
    skills: skillsStats
  };
}
